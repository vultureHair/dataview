...
export default {
    ...
    data() {
        return {
            option: {
                title: {
                    text: this.$t("boss.hotModel"),
                    left: "center",
                    subtext: this.$t("boss.hotSmallTitle")
                },
                tooltip: {
                    trigger: "axis",
                    axisPointer: {
    ...
    methods: {
        ...
        getData() {
            this.axios
                .get(this.GLOBAL.urlHead + "Boss/GetBossTopModel")
                .then(response => {
                    for (let i = 0; i < response.data.data.length; i++) {
                        this.option.xAxis[0].data[i] = response.data.data[i].model;
