<template>
    <div class="container-grid">
        <div id="GridStyleList" class="ag-theme-balham grid"></div>
    </div>
</template>
    ...
    methods: {
        async getData() {
            ...
            await this.axios
                .get(
                    this.GLOBAL.urlHead +
                        "ProductTotalTendency/GetCategoryGroupSaleInfo?vdoing=3&datetype=1&num=" +
                        this.period
                )
                .then(response => {
            ...
            await this.axios
                .get(
                    this.GLOBAL.urlHead +
                        "ProductTotalTendency/GetCategoryGroupSaleInfo?vdoing=3&datetype=2&num=" +
                        this.period
                )
                .then(response1 => {
            ...
            await this.axios
                .get(
                    this.GLOBAL.urlHead +
                        "ProductTotalTendency/GetCategoryGroupSaleInfo?vdoing=3&datetype=3&num=" +
                        this.period
                )
                .then(response => {
            ...
            //获取产品历史累积销售数据
            await this.axios
                .get(this.GLOBAL.urlHead + "ProductTotalTendency/GetCategoryGroupSaleInfo?vdoing=3")
                .then(response => {
                    this.amount = [];
                    ...
                    this.headerName.push("CategoryId");
                    this.headerName.push("CategoryName");
                })
                .catch(function(error) {
                    console.log(error);
                });
        ...
        initGrid() {
            ...
            let headLeft = [
                {
                    headerName: "",
                    children: [{ headerName: "style", field: "Id", pinned: "left" }]
                },
                {
            ...
            var gridOptions = {
                ...
                onRowDoubleClicked: function(event) {
                    window.location.href =
                        "http://data.ivalor.com/#/styleInfo?style=" + event.data.Id;
                }
            };
