...
export default {
    ...
    data() {
        return {
            option: {
                title: {
                    text: this.$t("style.layout.title"),
                    left: "center"
                },
                toolbox: {
                    orient: "vertical",
    ...
    methods: {
        ...
        getData() {
            ...
            this.axios
                .get(this.GLOBAL.urlHead + "StyleLayout/getStyleSalesBar?dayCount=" + this.dayCount)
                .then(response => {
                    for (let i = 0; i < response.data.data.length; i++) {
                        this.option.series[0].data[i] = response.data.data[i].quantity;

                        this.option.xAxis.data[i] = response.data.data[i].Styletype;
                    }
