...
export default {
    ...
    data() {
        return {
            option: {
                title: [
                    {
                        text: this.$t("category.layout.proTitle"),
                        left: "center",
                        top: "10px"
                    }
                ],
                ...
                xAxis: {
                    type: "category",
                    name: this.$t("category.layout.categoryName"),
                    data: []
                },
                yAxis: [
                    {
    ...
    methods: {
        ...
        getData() {
            this.axios
                .get(this.GLOBAL.urlHead + "CategoryLayout/GetCategoryProductAccount")
                .then(response => {
                    let tempData = response.data.data;

                    for (let i = 0; i < response.data.data.length; i++) {
                        this.option.xAxis.data[i] = tempData[i].CategoryName;
