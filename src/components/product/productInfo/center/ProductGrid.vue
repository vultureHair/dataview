<template>
    <div>
        ...
        <el-button type="primary" icon="el-icon-download" class="downloadData" @click="download()"
            >下载</el-button
        >
        <div id="GridCategory" style="width: 100%;height: 250px;" class="ag-theme-balham"></div>
    </div>
</template>
    ...
    methods: {
        getData() {
            ...
            this.axios
                .get(
                    this.GLOBAL.urlHead +
                        "ProductTotalTendency/GetProductGroupSaleInfo?upc=" +
                        this.barCode +
                        "&days=180"
                )
                .then(response => {
                    ...
                    temp[1] = response.data.data[0].BrandSale;
                    temp[2] = response.data.data[0].StyleTypeSale;
                    temp[3] = response.data.data[0].ModelSale;

                    for (let i = 0; i < 4; i++) {
                        temp[i].SalesRatio = parseFloat(temp[i].SalesRatio).toFixed(4) + "%";
                    }
                    this.tableData = temp;
                    this.initGrid();
                })
