<template>
    <div>
        <h3>{{ this.$t("product.info.salesList") }}</h3>
        <p class="showData">（{{ this.$t("product.info.salesData20") }}）</p>
        <div id="GridCenter" style="width: 100%;height: 410px;" class="ag-theme-balham"></div>
    </div>
</template>
    ...
    methods: {
        getData() {
            ...
            this.axios
                .get(
                    this.GLOBAL.urlHead +
                        "ProductDetail/GetProductDetailSales?barCode=" +
                        this.barCode
                )
                .then(response => {
