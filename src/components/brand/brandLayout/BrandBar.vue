...
export default {
    name: "CategoryTreeBar",
    data() {
        return {
            option: {
                title: {
                    text: this.$t("brand.layout.title"),
                    left: "center"
                },
                toolbox: {
                    orient: "vertical",
                ...
                xAxis: {
                    type: "category",
                    name: this.$t("brand.layout.brand"),
                    data: []
                },
                yAxis: {
                    type: "value",
    ...
    methods: {
        ...
        getData() {
            ...
            this.axios
                .get(this.GLOBAL.urlHead + "BrandLayout/getBrandSalesBar?dayCount=" + this.dayCount)
                .then(response => {
                    for (let i = 0; i < response.data.data.length; i++) {
                        this.option.series[0].data[i] = response.data.data[i].quantity;

                        this.option.xAxis.data[i] = response.data.data[i].brand;
                    }
